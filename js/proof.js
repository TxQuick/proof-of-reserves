...
const CSV_DELIMITER = ',';
const NEW_LINE = '\r\n';
const AUDIT_FILE_PATH_TEMPLATE = 'https://raw.githubusercontent.com/TxQuick/proof-of-reserves/main/audit_files/TxQuick_ProofOfReserves_PROOFDATE_merkletree.txt';
...
$(function() {
	...
	$('.inputProofSentence').on('input', function(event) {
		...
		/*
			This is the user string creation used by the TxQuick Exchange

			The TxQuick client identified as [userDataChunk]
		...
		if (i === 1) {
			...
			if (data[1]) {
				...
				$('.stepTwoStatus')
					.removeClass("boldGreen")
					.addClass("boldRed")
					.html('Error: Invalid TxQuick Merkle Tree file');
				return;
			}
		} else if (i === 2) {
			const data = row.split(':');
...
function generateMerkleTree(exchangeFileContent, exchangeFileName) {
		...
		if (data.length !== 3) {
			if (i === 1) {
				...
				if (data[1]) {
					...
					alert('Invalid TxQuick Source file.');
					return;
				}
			} else {
	...
	if (leaves.length > 0) {
		...
		let output = 'TxQuick Proof Of Reserves Merkle Tree File' + NEW_LINE;
